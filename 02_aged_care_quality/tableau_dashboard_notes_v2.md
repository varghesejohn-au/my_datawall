...
> **Status note (added post-build):** this doc reflects the original build
...
> **[BUILT DIFFERENTLY]** Built as two separate worksheets side by side —
> "Top Performers" (`benchmark_facilities`) and "Hidden Champions"
> (`facility_audit_intelligence`) — rather than one sheet with a toggle.
> Simpler to build and matches the rest of the dashboard's side-by-side
> layout pattern. Note: `benchmark_facilities` initially had a data-quality
...
> **[BUILT DIFFERENTLY]** Split into two separate sheets rather than one:
>
> - **Selected Facility - Verdict** — `rating_classification` plus a
>   `facility_lookup`-backed status label (originally "Audit Data Status:
>   HAS/NO 2026 AUDIT DATA", renamed to "2026 Star Rating: HAS/NO 2026
>   STAR RATING" for precision — the original wording implied general data
>   completeness rather than specifically whether an overall star rating
>   was published). Verdict's primary data source is `facility_lookup`
>   (one row per facility, always present) rather than
>   `facility_audit_intelligence` directly, specifically so the status
>   label still renders for facilities with zero matching rows elsewhere.
> - **Selected Facility - Audit Status** — the numeric measures: Avg
>   Overall Stars, Avg Outcome Composite, Avg Predicted Outcome, Avg
>   Residual, Avg Compliance Stars, Avg Quality Measures Stars, and the 5
>   risk flags as counts.
>
> **No-data handling (added post-build, not in original plan):** facilities
> with no 2026 record at all render fully blank on Facility Drill-Down,
> Resident Experience, and Audit Status — Detail-shelf fields don't render
> in a sheet's title when the primary source has zero matching rows, since
> there's no mark to attach them to. Verdict is unaffected by this because
> its primary source is `facility_lookup`, not the detail tables. A fix
> was built for Facility Drill-Down only (a small reusable
> `facility_lookup`-based worksheet floated over the panel, since that
> data source always has a row) — deliberately not extended to Resident
> Experience or Audit Status, as a scoped decision, not an oversight.
